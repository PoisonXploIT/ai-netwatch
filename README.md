...
- Las conexiones UDP (QUIC/HTTP-3) se registran via `netstat -ano -p UDP` (remoto+PID): `Get-NetUDPEndpoint` no reporta remotos en sockets UDP conectados y Sysmon de esta maquina no emite EID3 para msedge. Limitacion medida: netstat omite sockets UDP IPv6, asi que QUIC v4 a destino IA se ve completo (proceso + SNI) y QUIC IPv6 queda ciego de conexion (el SNI se captura pero sin evento al que anclarlo).
